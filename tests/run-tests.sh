...
test_php_version_file_detection() {
    test_case "Install PHP 8.2 for .php-version test"
    phpswitcher install 8.2
    assert_success $? "phpswitcher install 8.2"
    ...
    (
        ...
        local active_version
        active_version=$(php --version | head -n 1 | grep -o -E '[0-9]+\.[0-9]+' | head -n 1)
        assert_contains "$active_version" "8.2" "Active version is 8.2 after detection"
    )
    rm -rf test_project
}
